$(document).on("pagecreate","#shop",function(event){
	var startNorth;
	var startEast;
...
//	var nowPosition;
	...
	//get location and set map center
	
	function getCurrentLocation() {
		if (navigator.geolocation) {//有抓到使用者位置，以使用者為中心
			navigator.geolocation.getCurrentPosition(showCurrentPosition);
		} else {                    //沒抓到使用者位置，以總店為中心
			divLocation.innerHTML = "Geolocation is not supported.";
			noNowPosition();
		}
	}
	...
	function showCurrentPosition(position) {
		...
		var hint = document.getElementById("mapInfo"); //display mapInfo
		hint.innerHTML = "現在位置：("+position.coords.latitude+","+position.coords.longitude+")";
		...
		locations.push(userMaker);//push marker, get .position
		nextStep();
	}
	
	function noNowPosition(){
		var mapProp = {
			center : myCenter,
			zoom : 13,
			mapTypeId : google.maps.MapTypeId.ROADMAP
...
//set marker
	 ...
	 // marker click event
	  	google.maps.event.addListener(marker, 'click', function() {
	  		if(haveUserLocation){
		  		if (locations.length>1){ //如果location[]有兩個元素，丟掉最後一個(第一個是使用者位置)
		  			locations.pop();
		  		} 
	  		}else{
	...
	//	    } else {
	//	    	hint.innerHTML = "Geolocation is not supported by this browser.";
	//	    }
	//	}
	//
	...
	//		hint.innerHTML = "現在位置：("+startNorth+","+startEast+")";		
	//	}
	//
