...
//儲存訂單項目
function order_item (name , quantity , capacity , sugar , ice) {
     this.name = name;
...
$("#joinOrder").click(function () {
    ...
    if(isCorrect){
        //送出資料
        var order_product = 
            new order_item(order_item_info.name,$("#orderNumber").val(),label_capacity.text(),label_suger.text(),label_ice.text());
        // console.log(test.name + ':' + test.quantity + ':' + test.capacity + ':' + test.sugar + ':' + test.ice );
...
function createOrderList (order_product) {
    ...
    var $h3 = $('<h3>');  
    $h3.text(order_product.name + ':' + order_product.quantity + ':' + order_product.capacity + ':' + order_product.sugar + ':' + order_product.ice);
